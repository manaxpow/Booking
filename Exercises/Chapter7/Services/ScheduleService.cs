...
public class ScheduleService : IScheduleService
{
    ...
    public async Task<ServiceResult<GetScheduleWithDetailsByIdResponse>> GetByIdAsync(int id)
    {
        ...
        if (schedule is null)
        {
            return ServiceResult<GetScheduleWithDetailsByIdResponse>.NotFound(
                $"Không tìm thấy lịch chạy id={id}."
            );
        }

        var summary = MapScheduleSummaryResponse(schedule);
        return ServiceResult<GetScheduleWithDetailsByIdResponse>.Success(
            new GetScheduleWithDetailsByIdResponse
            {
                Id = summary.Id,
                CarId = summary.CarId,
    ...
    public async Task<ServiceResult<CreateScheduleResponse>> CreateScheduleAsync(
    ...
    )
    {
        ...
        {
            return ServiceResult<CreateScheduleResponse>.BadRequest(
        ...
        {
            return ServiceResult<CreateScheduleResponse>.BadRequest(
                $"Xe id={request.CarId} không tồn tại."
            );
        }
        ...
        {
            return ServiceResult<CreateScheduleResponse>.BadRequest(
                $"Tài xế id={request.DriverId} không tồn tại."
            );
        }

        var pickupDestination = await _unitOfWork.Destinations.GetByIdAsync(
            request.PickupDestinationId
        );
        if (pickupDestination is null)
        {
            return ServiceResult<CreateScheduleResponse>.BadRequest(
                $"Điểm đón id={request.PickupDestinationId} không tồn tại."
            );
        }

        var dropoffDestination = await _unitOfWork.Destinations.GetByIdAsync(
            request.DropoffDestinationId
        );
        if (dropoffDestination is null)
        {
            return ServiceResult<CreateScheduleResponse>.BadRequest(
                $"Điểm trả id={request.DropoffDestinationId} không tồn tại."
            );
        }
        ...
        if (hasConflict)
        {
            return ServiceResult<CreateScheduleResponse>.Conflict(
                "Schedule bị conflict với lịch hiện tại. (BT8.1 sẽ triển khai chi tiết)"
            );
        }
        ...
        {
            return ServiceResult<CreateScheduleResponse>.BadRequest(
                $"Không lấy được thông tin ghế của xe id={request.CarId}."
            );
        }
        ...
        var createdSchedule = await _unitOfWork.Schedules.GetScheduleWithDetailsByIdAsync(
            schedule.Id
        );
        var summary = MapScheduleSummaryResponse(createdSchedule!);
        return ServiceResult<CreateScheduleResponse>.Success(
    ...
    public async Task<ServiceResult> DeleteScheduleAsync(int id)
    {
        ...
        var seatBookings = await _unitOfWork.SeatBookings.FindAsync(sb =>
            sb.ScheduleId == id && sb.IsBooking
        );
        if (seatBookings.Any())
        {
    ...
    public async Task<ServiceResult<UpdateScheduleStatusResponse>> UpdateScheduleStatusAsync(
        int id,
    ...
    )
    {
        ...
        {
            return ServiceResult<UpdateScheduleStatusResponse>.BadRequest(
                "Status không hợp lệ. Chỉ chấp nhận: ACTIVE, INACTIVE, FINISH."
            );
        }
        ...
        if (schedule is null)
        {
            return ServiceResult<UpdateScheduleStatusResponse>.NotFound(
                $"Không tìm thấy lịch chạy id={id}."
            );
        }
        ...
        {
            return ServiceResult<UpdateScheduleStatusResponse>.BadRequest(
                "Schedule đã FINISH (terminal), không thể chuyển trạng thái khác."
            );
        }
        ...
        {
            return ServiceResult<UpdateScheduleStatusResponse>.BadRequest(
                "Chỉ được chuyển sang FINISH từ ACTIVE."
            );
        }
        ...
        return ServiceResult<UpdateScheduleStatusResponse>.Success(
            new UpdateScheduleStatusResponse
            {
                Id = summary.Id,
                CarId = summary.CarId,
                DriverId = summary.DriverId,
