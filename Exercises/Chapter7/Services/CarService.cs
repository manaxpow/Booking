...
public class CarService : ICarService
{
    ...
    public async Task<ServiceResult<GetCarByIdResponse>> GetByIdAsync(int id)
    {
        ...
        return ServiceResult<GetCarByIdResponse>.Success(
            new GetCarByIdResponse
            {
                Id = summary.Id,
    ...
    public async Task<ServiceResult<IEnumerable<CarSummaryResponse>>> GetAllAsync()
    {
        var cars = await _unitOfWork.Cars.GetAllAsync();
        var data = cars.Select(MapCarSummaryResponse);
        return ServiceResult<IEnumerable<CarSummaryResponse>>.Success(data);
    }
    ...
    public async Task<ServiceResult<CreateCarResponse>> CreateAsync(CreateCarRequest request)
    {
        ...
        {
            return ServiceResult<CreateCarResponse>.Conflict(
                $"Biển số '{request.LicensePlate}' đã tồn tại."
            );
        }
        ...
        var summary = MapCarSummaryResponse(car);
        return ServiceResult<CreateCarResponse>.Success(
            new CreateCarResponse
            {
                Id = summary.Id,
                LicensePlate = summary.LicensePlate,
                Brand = summary.Brand,
