...
public class DestinationService(IUnitOfWork unitOfWork) : IDestinationService
{
    ...
    public async Task<ServiceResult<CreateDestinationResponse>> CreateAsync(
    ...
    )
    {
        ...
        {
            return ServiceResult<CreateDestinationResponse>.BadRequest(
                "Tên destination không được để trống."
            );
        }

        var existed = await _unitOfWork.Destinations.FindAsync(d =>
            d.Name.ToLower() == normalizedName.ToLower()
        );
        if (existed.Any())
        {
            return ServiceResult<CreateDestinationResponse>.Conflict(
                $"Destination '{normalizedName}' đã tồn tại."
            );
        }
        ...
        return ServiceResult<CreateDestinationResponse>.Success(
            new CreateDestinationResponse { Id = destination.Id }
        );
    }
    ...
    public async Task<ServiceResult<GetDestinationByIdResponse>> GetByIdAsync(int id)
    {
        ...
        return ServiceResult<GetDestinationByIdResponse>.Success(
            new GetDestinationByIdResponse
            {
                Id = destination.Id,
